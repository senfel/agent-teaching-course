#!/usr/bin/env python3
"""step10_team.py — 第 10 期:Agent Team 团队协作

原理一句话:主 Agent 通过 team_wake 唤醒持久队友(有 inbox + 持久 messages),
队友之间通过 send_message / broadcast_message 异步通信,实现"前端组+后端组"
协作开发一个功能 = Agent Team 机制。

本期目标:
1. 解决第 09 期埋的伏笔:Alex 有了子代理能并发派遣,但子代理是"临时工"
   ——派遣→运行→结果→销毁,没有持久状态,队友之间无法直接通信。遇到需要
   多轮协作的场景(如"前端等后端的接口定义再继续")就卡住了
2. 引入 Agent Team 机制 + 五个新增工具:
   - team_wake           — 唤醒持久队友(上下文跨唤醒保持,inbox 消息注入)
   - send_message         — 给指定队友的 inbox 发消息(主 Agent 或队友都能调)
   - broadcast_message    — 向所有队友的 inbox 广播一条消息
   - read_inbox           — 读取某个队友的 inbox(查看消息)
   - list_team            — 查看团队成员、inbox 消息数、唤醒次数
3. 三个核心概念:
   - 持久队友:team member 的 messages 和 inbox 跨唤醒保持,不像 subagent 用完即弃
     → 前端队友第二次被唤醒时,还记得第一次做了什么
   - inbox 消息系统:每个队友有 inbox(消息队列),队友之间异步通信
     → 前端队友给后端队友发消息"我需要登录接口的格式",后端队友下次被唤醒时看到
   - 消息广播:broadcast_message 一次性给所有队友的 inbox 发消息
     → "大家注意,需求变更了"一条消息所有人收到
4. 两个预定义团队角色:
   - frontend_dev(前端开发)— 有 inbox + 持久 messages,能给其他队友发消息
   - backend_dev(后端开发)— 有 inbox + 持久 messages,能给其他队友发消息
5. 关键差异(Subagent vs Team):
   - Subagent(第 09 期):临时、无状态、单向通信(主→子→结果)
   - Team Member(本期):持久、有状态、双向通信(inbox 异步消息)
6. 演示场景:前端组+后端组协作开发"用户登录功能"
   - Alex 拆任务 → 唤醒前端 → 前端给后端发消息要接口 → 唤醒后端 → 后端回复 → 再唤醒前端对接

累积式:step10 = step09 + Agent Team
(保留 step09 的三层记忆、技能层、工具循环、TodoList、子代理全部逻辑,
 只新增 Team 层 + 5 个工具)

运行:
    python code/step10_team.py
"""
import os
import json
import ast
...
# ============ 8. 工具定义(第 09 期 11 个 + 本期新增 5 个 = 16 个)============
TOOLS = [
    {
        "type": "function",
        "function": {
...
def execute_tool(name: str, arguments: dict) -> str:
    ...
    if name == "team_wake":
        ...
        return team_wake(member_id, task)
...
# ============ 10. 工具循环(与第 09 期完全一致,Team 工具也复用它)============
...
def main():
    print("=" * 60)
    print("第 10 期:Agent Team 团队协作 —— 让 Alex 组建持久团队")
    print("=" * 60)
    print(f"当前模型: {MODEL}")
    ...
    print(f"团队成员: {', '.join(TEAM_MEMBERS.keys())}")
    ...
    # 拼装 system prompt:人设 + 技能索引 + 用户画像 + TodoList 指南 + Subagent 指南 + Team 指南
    system_prompt = soul
    if skills_prompt:
        system_prompt += "\n" + skills_prompt
    if user_profile:
    ...
    # Agent Team 工作指南(内联,注入 system prompt)
    team_guide = (
        "\n\n## Agent Team 使用指南\n"
        "你可以通过 team_wake 唤醒持久队友。队友的对话历史和 inbox 跨唤醒保持。\n"
        "队友之间可以通过 send_message 互发消息,通过 broadcast_message 广播消息。\n"
        "决策流程:\n"
        "1. 当任务需要多个角色多轮协作时(如前端+后端合作开发),用 team_wake 逐个唤醒\n"
        "2. 当需要给队友传话时,用 send_message 发到对方 inbox\n"
        "3. 当需要通知所有人时,用 broadcast_message 广播\n"
        "4. 当需要查看队友状态时,用 list_team\n"
        "5. 当需要查看队友收到的消息时,用 read_inbox\n"
        "关键差异:子agent 是临时的(用完即弃),队友是持久的(上下文跨唤醒保持)。\n"
        "使用模式:唤醒前端 → 前端给后端发消息要接口 → 唤醒后端 → 后端回复 → 再唤醒前端对接。\n"
    )
    system_prompt += team_guide
    print(f"[Team 指南] 内联注入({len(team_guide)} 字符)")
    ...
    # ============ 自动演示:前端组+后端组协作开发"用户登录功能" ============
    print('[自动演示] 前端组+后端组协作开发"用户登录功能":\n')

    demo_messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": (
            "请帮我组建团队协作开发一个用户登录功能,具体步骤:\n"
            "1. 先唤醒后端开发(frontend_dev),让后端设计登录接口\n"
            "2. 再唤醒前端开发(frontend_dev),让前端根据后端的接口对接页面\n"
            "3. 如果前端有问题,通过 send_message 给后端发消息,再唤醒后端回复\n"
            "注意:团队成员之间需要通过 inbox 消息沟通,模拟真实的团队协作。"
        )},
    ]
    try:
    ...
    demo_messages2 = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": (
            "请帮我做一件事:用 broadcast_message 通知所有团队成员,"
            "'需求变更:登录功能需要增加验证码校验',"
            "然后用 list_team 查看所有人的 inbox 状态,"
            "最后唤醒 backend_dev 确认收到通知。"
        )},
    ]
    try:
    ...
    # ============ 自由对话(累积式) ============
    print("\n现在进入自由对话(Alex + 人设 + 技能库 + 长期记忆 + TodoList + 子代理 + Agent Team)")
    ...
    while True:
        ...
        if user_input.lower() == "soul":
            print(f"[人设 + 技能 + 用户画像 + TodoList 指南 + Subagent 指南 + Team 指南]:\n{system_prompt[:800]}...\n")
            continue
        ...
        if user_input.lower() == "wake":
            ...
            for mid, config in TEAM_MEMBERS.items():
                ...
                wakes = config["wake_count"]
                has_ctx = "有" if config["messages"] is not None else "无"
                print(f"  - {mid}: {config['description']}")
                print(f"    inbox={inbox_n} 未读  唤醒={wakes} 次  上下文={has_ctx}")
                print(f"    允许工具: {', '.join(config['allowed_tools'])}")
            print("\n[提示] 问 Alex 一个问题,如'让前端和后端协作开发登录功能',它会自己组队。")
            continue
        if not user_input:
            continue
        ...
        try:
            ...
            # 如果团队有活动,打印团队面板
            team_active = any(
                cfg["wake_count"] > 0 or cfg["inbox"]
                for cfg in TEAM_MEMBERS.values()
            )
            if team_active:
                print_team_panel()
        except Exception as e:
